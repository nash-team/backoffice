...
from backoffice.domain.entities.ebook import BackCoverConfig, KDPExportConfig, inches_to_px
...
from backoffice.infrastructure.utils.color_utils import (
    TEXT_BLACK_CMYK,
    convert_rgb_to_cmyk,
)
from backoffice.infrastructure.utils.spine_generator import get_font_path, load_font_safe
...
class OpenRouterImageProvider(CoverGenerationPort, ContentPageGenerationPort):
    ...
    def _get_theme_primary_color(self, theme: str) -> tuple[int, int, int]:
        ...
        from backoffice.domain.entities.theme_profile import load_theme_from_yaml
    ...
    def _add_back_cover_text(
        ...
        draw.rectangle(
            [
                (barcode_x - QUIET_ZONE, barcode_y - QUIET_ZONE),
                (barcode_x + BARCODE_WIDTH + QUIET_ZONE, barcode_y + BARCODE_HEIGHT + QUIET_ZONE),
            ],
