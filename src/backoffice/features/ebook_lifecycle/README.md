...
The `ebook_lifecycle` feature is a bounded context that manages the lifecycle states of ebooks (DRAFT → APPROVED → REJECTED) and provides approval/rejection workflows with file storage integration.
...
```python
from backoffice.features.ebook_lifecycle.domain.usecases.get_stats_usecase import (
    GetStatsUseCase,
)
...
```bash
pytest src/backoffice/features/ebook_lifecycle/tests/unit/ -v
```

### Run Integration Tests

```bash
pytest src/backoffice/features/ebook_lifecycle/tests/integration/ -v
```
